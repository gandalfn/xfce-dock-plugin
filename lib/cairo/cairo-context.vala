...
public class Xdp.CairoContext : Cairo.Context
{
    // static methods
    ...
    public void
    set_source_color (CairoColor inColor)
    {
        set_source_rgba(inColor.red, inColor.green, inColor.blue, inColor.alpha);
    }
     ...
     */
    public void
    diagonal_gradient(CairoColor inColor, CairoColor inShadeColor,
                      double inW, double inH, bool inInvert, double inVariance)
    {
        ...
        {
            start = inShadeColor;
            start.shade(1 - inVariance);
            end = inColor;
            end.shade(1 + inVariance);
        }
        else
        {
            end = inShadeColor;
        ...
        CairoPattern linearGradient = new CairoPattern.linear(inW * 0.5 - offset, 0,
                                                              inW * 0.5 + offset, inH);
        linearGradient.add_color_stop (0.0, start);
        linearGradient.add_color_stop (0.4, start);
        linearGradient.add_color_stop (0.5, inColor);
        linearGradient.add_color_stop (0.6, end);
        linearGradient.add_color_stop (1.0, end);
        set_source(linearGradient);
    }

    public void
    shade_text(string inText, Pango.FontDescription inFtDesc,
               Pango.Alignment inAlignment, CairoColor inColor,
               CairoColor inShadeColor, double inAlpha, bool inInvert)
    {
        Pango.Layout layout = Pango.cairo_create_layout(this);
        int height, width;
        double x, y, delta;
        CairoColor start = inShadeColor, end = inShadeColor, color = inColor;
    ...
    public void
    button (CairoColor inBg, CairoColor inColorActive, CairoColor inColorInactive,
            ...
            double inBorder, double inProgress, double inVariance)
    {
        ...
        if (inProgress < 1.0)
        {
            ...
            radialGradient.add_color_stop_alpha(0.1, inColorInactive, 1.0);
            radialGradient.add_color_stop_alpha(1.0, inColorInactive, 0.1);
            ...
            linearGradient.add_color_stop_alpha(0.1, inColorInactive, 1.0);
            linearGradient.add_color_stop_alpha(1.0, inColorInactive, 0.1);
            set_source(linearGradient);
            ...
            set_source_color (inColorInactive);
        ...
        if (inProgress > 0.0)
        {
            ...
            //corners
            CairoPattern radialGradient = new CairoPattern.radial(inBorder, inBorder, 0.0, inBorder, inBorder, inBorder);
            radialGradient.add_color_stop_alpha(0.1, inColorActive, 1.0);
            radialGradient.add_color_stop_alpha(1.0, inColorActive, 0.1);
            set_source(radialGradient);
            rounded_rectangle(0, 0, inBorder, inBorder, inBorder, CairoCorner.TOPLEFT);
            fill ();
            ...
            // Borders
            CairoPattern linearGradient = new CairoPattern.linear(inBorder, inHeight / 2.0, 0, inHeight / 2.0);
            linearGradient.add_color_stop_alpha(0.1, inColorActive, 1.0);
            linearGradient.add_color_stop_alpha(1.0, inColorActive, 0.1);
            set_source(linearGradient);
            rectangle(0, inBorder, inBorder, inHeight - (inBorder*2));
            fill ();
            ...
            // And finally the real background
            set_source_color (inColorActive);
            rectangle(inBorder, inBorder, inWidth - (inBorder * 2), inHeight - (inBorder * 2));
            fill ();
        }
